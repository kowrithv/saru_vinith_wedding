...
import {
  Calendar, MapPin, Clock, ArrowRight, BookOpen,
  Camera, MessageSquare, Heart, HelpCircle, Gamepad2,
} from 'lucide-react'
import Button from '@/components/ui/Button'
import SectionTitle from '@/components/ui/SectionTitle'
import Card from '@/components/ui/Card'
import { cn } from '@/lib/utils'
import { receptionEvent, traditionalWedding, storyTimeline, showTraditionalWeddingTab } from '@/lib/config'
      ...
      <section className="py-16 md:py-24 bg-champagne-light/40">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          >
            <motion.div variants={fadeInUp}>
              <SectionTitle
                title={showTraditionalWeddingTab ? 'Die Hochzeiten' : 'Der Empfang'}
                subtitle={
                  showTraditionalWeddingTab
                    ? 'Zwei besondere Feiern – eine Liebe. Alle wichtigen Informationen auf einen Blick.'
                    : 'Alle wichtigen Informationen auf einen Blick.'
                }
              />
            </motion.div>

            <div className={cn('grid grid-cols-1 gap-8', showTraditionalWeddingTab && 'md:grid-cols-2')}>
              {/* Reception Card */}
              <motion.div variants={fadeInUp} className={showTraditionalWeddingTab ? undefined : 'md:max-w-md md:mx-auto md:w-full'}>
                <Card variant="dark" padding="lg" className="h-full group">
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <div>
                      <p className="text-champagne/60 text-xs font-medium uppercase tracking-wider">Empfang</p>
                      <h3 className="font-serif text-xl font-semibold text-white">{receptionEvent.title}</h3>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-3 text-white/80">
                      ...
                      <span className="text-sm">{receptionEvent.displayDate}</span>
                    </div>
                    <div className="flex items-center gap-3 text-white/80">
                      <Clock size={16} className="text-gold flex-shrink-0" />
                      <span className="text-sm">{receptionEvent.time}</span>
                    </div>
                    <div className="flex items-start gap-3 text-white/80">
                      <MapPin size={16} className="text-gold flex-shrink-0 mt-0.5" />
                      <div>
                        <p className="text-sm font-medium text-white">{receptionEvent.venue}</p>
                        <p className="text-xs text-white/60">{receptionEvent.address}</p>
                      </div>
                    </div>
                  </div>
